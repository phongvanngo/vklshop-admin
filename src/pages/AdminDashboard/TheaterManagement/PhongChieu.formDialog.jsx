...
import FilterRoomType from "./PhongChieu.formDialog.filterRoomType";
...
export default function PhongChieuFormModal() {
  ...
  useEffect(() => {
    clearErrors("name");
    if (defaultData?.id) {
      const { name, information, roomTypeId } = defaultData;
      setValue("name", name);
      setSelectedTheaterSytem(
        listRoomType.find((element) => element.id === roomTypeId) || {}
      );
    } else {
      setValue("name", "");
    }
