...
export default function FilterMinute({ selected, setSelected }) {
  let listMinute = [];
  for (let i = 0; i <= 59; i++) listMinute.push(i);
  // const [selected, setSelected] = useState(listMinute[0]);
  return (
    <div className="w-28">
        ...
        <div className="relative mt-1">
          ...
          >
            <Listbox.Options className="absolute w-full z-20 py-1 mt-1 overflow-y-scroll max-h-50 text-base bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {listMinute.map((minute, minuteIdx) => (
                <Listbox.Option
                  key={minuteIdx}
                  className={({ active }) =>
                    `${
